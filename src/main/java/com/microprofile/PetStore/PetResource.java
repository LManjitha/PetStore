...
public class PetResource {
    ...
    public Response getPets() {
        return Response.ok(PetSingleton.getInstance().getArrayList()).build();
    }
    ...
    public Response searchPet(@DefaultValue("-1") @QueryParam("id") int petId,
        ...
        if(petId != -1 && petName.equals("null") && petAge == 0){
            ...
            for (int i=0;i<PetSingleton.getInstance().getArrayList().size();i++){
                if(petId == PetSingleton.getInstance().getArrayList().get(i).getPetId()){
                    isPetFound = true;
                    id = i;
                }
            }
            ...
            for (int i=0;i<PetSingleton.getInstance().getArrayList().size();i++){
                if(petName.equals(PetSingleton.getInstance().getArrayList().get(i).getPetName())){
                    isPetFound = true;
                    id = i;
                }
            }
            ...
            for (int i=0;i<PetSingleton.getInstance().getArrayList().size();i++){
                if(petAge == PetSingleton.getInstance().getArrayList().get(i).getPetAge()){
                    isPetFound = true;
                    id = i;
                    temp.add(PetSingleton.getInstance().getArrayList().get(id));
                }
            }
            if(isPetFound){
    ...
    public Response addPet(String petData) throws JSONException {
        ...
        if(jsonObject.has("name") && jsonObject.has("age") && jsonObject.has("type")){
            ...
            pet.setPetId(PetSingleton.getInstance().getArrayList().get(PetSingleton.getInstance().getArrayList().size()-1).getPetId()+1);
            pet.setPetName(petName);
            pet.setPetAge(petAge);
            pet.setPetType(petType);
            pets.add(pet);
            temp.addAll(PetSingleton.getInstance().getArrayList());
            temp.addAll(pets);
            PetSingleton.getInstance().setArrayList(temp);
            //petData.addData(pet);
...
//            err.put("successful",err1);
    ...
    public Response deletePet(@PathParam("petId") int petId){
        ...
        for (int i=0;i<PetSingleton.getInstance().getArrayList().size();i++){
            if(petId == PetSingleton.getInstance().getArrayList().get(i).getPetId()){
                PetSingleton.getInstance().getArrayList().remove(i);
                isPetFound = true;
            }
        }
        if(isPetFound){
    ...
    public Response editPet(@PathParam("petId") int petId,String petData) throws JSONException {
        ...
        if(jsonObject.has("name")){
            for (int i=0;i<PetSingleton.getInstance().getArrayList().size();i++){
                if(petId == PetSingleton.getInstance().getArrayList().get(i).getPetId()){
                    PetSingleton.getInstance().getArrayList().get(i).setPetName(jsonObject.getString("name"));
                    isUpdated=true;
                    id = i;
                }
            }
        ...
        if(jsonObject.has("age")){
            for (int i=0;i<PetSingleton.getInstance().getArrayList().size();i++){
                if(petId == PetSingleton.getInstance().getArrayList().get(i).getPetId()){
                    PetSingleton.getInstance().getArrayList().get(i).setPetAge(Integer.parseInt(jsonObject.getString("age")));
                    isUpdated=true;
                    id = i;
                }
            }
        ...
        if(jsonObject.has("type")){
            for (int i=0;i<PetSingleton.getInstance().getArrayList().size();i++){
                if(petId == PetSingleton.getInstance().getArrayList().get(i).getPetId()){
                    PetSingleton.getInstance().getArrayList().get(i).setPetType(jsonObject.getString("type"));
                    isUpdated=true;
                    id = i;
                }
            }
