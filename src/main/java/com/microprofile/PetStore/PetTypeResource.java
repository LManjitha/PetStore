...
public class PetTypeResource {
    ...
    public Response getPetTypes(){
        return Response.ok(PetTypeSingleton.getInstance().getArrayList()).build();
    }
    ...
    public Response addPetType(String petTypeData) throws JSONException {
        ...
        if(jsonObject.has("petType") && jsonObject.has("petsQuantity")){
            ...
            petType.setPetTypeId(PetTypeSingleton.getInstance().getArrayList().get(PetTypeSingleton.getInstance().getArrayList().size()-1).getPetTypeId()+1);
            petType.setPetType(petTypeName);
            petType.setPetsQuantity(petsQuantity);
            petTypes.add(petType);
            temp.addAll(PetTypeSingleton.getInstance().getArrayList());
            temp.addAll(petTypes);
            PetTypeSingleton.getInstance().setArrayList(temp);
            //petData.addData(pet);
    ...
    public Response deletePetType(@PathParam("petTypeId") int petTypeId){
        ...
        for (int i=0;i<PetTypeSingleton.getInstance().getArrayList().size();i++){
            if(petTypeId == PetTypeSingleton.getInstance().getArrayList().get(i).getPetTypeId()){
                PetTypeSingleton.getInstance().getArrayList().remove(i);
                isPetFound = true;
            }
        }
        if(isPetFound){
    ...
    public Response editPetType(@PathParam("petTypeId") int petTypeId,String petTypeData) throws JSONException {
        ...
        if(jsonObject.has("petType")){
            for (int i=0;i<PetTypeSingleton.getInstance().getArrayList().size();i++){
                if(petTypeId == PetTypeSingleton.getInstance().getArrayList().get(i).getPetTypeId()){
                    PetTypeSingleton.getInstance().getArrayList().get(i).setPetType(jsonObject.getString("petType"));
                    isUpdated=true;
                    id=i;
                }
            }
        ...
        if(jsonObject.has("petsQuantity")){
            for (int i=0;i<PetTypeSingleton.getInstance().getArrayList().size();i++){
                if(petTypeId == PetTypeSingleton.getInstance().getArrayList().get(i).getPetTypeId()){
                    PetTypeSingleton.getInstance().getArrayList().get(i).setPetsQuantity(Integer.parseInt(jsonObject.getString("petsQuantity")));
                    isUpdated=true;
                    id=i;
                }
            }
        ...
        if(isUpdated){
            return Response.ok(PetTypeSingleton.getInstance().getArrayList().get(id)).build();
        }else{
            return Response.ok("{\n" + "\"successful\":false\n" + "}").build();
        }
